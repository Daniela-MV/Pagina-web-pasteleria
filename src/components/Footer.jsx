export const Footer = () => (
	<footer className="footer mt-auto py-3 text-center" id="footerWeb">
		<div className="mainContainer">
			<div className="dataBakery">
				<p><b>Estamos ubicados en:</b> Av. Luis Vega y Morroy 230, zona dos extendida,
				Balaustradas, 76079 Santiago de Querétaro, Qro. </p>
				<br />
				<p><b>Horarios de atención:</b> L-V de 7:30 am - 20:00 pm y Sábado de 8:30am - 18:30pm </p>
				<br />
				<p><i class="fa-solid fa-phone-flip"></i> 4426071353</p>
				<br />
				<p><i class="fa-solid fa-envelope"></i> casamarquesa@gmail.com</p>
			</div>
			<div className="socialMedia">
				<p><b>Búcanos en nuestras redes como:</b></p>
				<br />
				<p></p>

			</div>
		</div>
	</footer>
);